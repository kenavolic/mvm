...
TEST_F(base_vm_test, assemble_single_instr) {
  check_assembler_ok(vm1, "a 1", {{0x0, 0x1, 0x0, 0x0, 0x0}});
...
TEST_F(base_vm_test, assemble_prog) {
  check_assembler_ok(
      vm1, "a 1\nb\nc 256\nd 1024\ne\nf\ng 1",
      {{0x0, 0x1, 0x0, 0x0, 0x0, 0x1, 0x2, 0x0, 0x1, 0x0, 0x0, 0x3,
...
TEST_F(base_vm_test, assemble_bad) {
  ...
  check_assembler_nok(vm1, "a 1 2", status_type::BAD_INSTR_OPERAND);
...
TEST_F(base_vm_test, interpret_prog) {
  prog_chunk c;
  // e, g 1, nop, a 1, f
  c.code = {0x4, 0x6, 0x1, 0x0, 0x0, 0x0, 0x90, 0x0, 0x1, 0x0, 0x0, 0x0, 0x5};
  EXPECT_EQ(vm1.interpret(std::move(c)), status_type::SUCCESS);
